...
def test_create_player():
    player_to_create = {"firstname": "Name",
                        ...
                        "position": "Midfield",
                        "dorsal": 5,"team_id": 1}
    response = client.post("/players/", json=player_to_create)
    json_response = response.json()
    # delete_created_db_record(PlayerDB, json_response.pop("id"))
...
def test_update_player():
    ...
    response = client.patch(f'/players/{player_id}', json=update_data)
    assert response.status_code == 200
